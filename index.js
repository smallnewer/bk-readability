...
(function () {
	...
	$R.getContent__find = function() {
		//  get content
		//  ===========
		var _found = $R.getContent__findInPage($R.win),
		...
		//  获取title
		//  html中已经有title了？
		// 从分析出的html中对比document的title，如果有则把相应的标签换成h1
		...
		if ($R.articleTitle > '');
			...
			(function() {
				...
				while (true) {
					...
					case(_prevNode.tagName && (_prevNode.tagName.toLowerCase() == 'body')) :
						// 如果是不同的标签，让prev等于first的标签。不知道prev和above的区别
					...
					if (_prevNode.previousSibling);
						...
						// 则等于父标签，继续下一次循环。大概就是循环父标签-父的前面兄弟-爷爷-爷爷的前面兄弟
						// 大体意思就是把主要文本标签前面的所有标签都便利，一直到body停止。
					...
					//  finished?
					// 结束？可能是文本超出65*3*3？或者已经有了title，就跳出循环
					switch (true) {
					case($R.measureText__getTextLength(_aboveHTML.replace(/<[^>]+?>/gi, '').replace(/\s+/gi, ' ')) > (65 * 3 * 3)) : case($R.getContent__find__hasIsolatedTitleInHTML(_aboveHTML)):
						return;
					}
				}
			...
			case(_differentTargets && (_aboveHTML.split('<a ').length < 3) && ($R.measureText__getTextLength(_aboveHTML.replace(/<[^>]+?>/gi, '').replace(/\s+/gi, ' ')) < (65 * 3))) : _foundHTML = _aboveHTML + _foundHTML;
				break;
			...
			if ($R.articleTitle > '');
				...
				(function() {
					...
					//  =========
					_foundHTML = '' + $R.articleTitleMarker__start + (_doc_title_parts[0].split(/\s+/i).length > 1 ? _doc_title_parts[0] : _documentTitle) + $R.articleTitleMarker__end + _foundHTML;
	...
	$R.getContent__nextPage__find = function(_currentPageWindow, _linksInCurrentPage) {
		...
		(function() {
			...
			for (var i = 0,
			_i = _possible.length; i < _i; i++) {
				for (var j = 0,
				_j = $R.nextPage__captionKeywords.length; j < _j; j++) {
					if (_possible[i]._caption.indexOf($R.nextPage__captionKeywords[j]) > -1) {
						...
						//  ============
						for (var z = 0,
						_z = $R.nextPage__captionKeywords__not.length; z < _z; z++) {
		...
		(function() {
			...
			_i = _possible.length; i < _i; i++) {
				...
				if ($R.measureText__getTextLength(_possible[i]._caption) <= 2);
				else {
					continue;
				}
	...
	$R.getContent__exploreNodeAndGetStuff = function(_nodeToExplore, _justExploring) {
		...
		var _recursive = function(_node) {
			...
			//  ===========
			switch (true) {
			case((_tag_name == '#invalid')) : case(($R.parsingOptions._elements_ignore.indexOf('|' + _tag_name + '|') > -1)) : return;
			...
			case ($R.parsingOptions._elements_self_closing.indexOf('|' + _tag_name + '|') > -1) : switch (true) {
			...
			case(_tag_name == 'a') : var _href = _node.href;
				...
				//  skip
				for (var i = 0,
				_i = $R.skipStuffFromDomains__links.length; i < _i; i++) {
			...
			case (_tag_name == 'img') :
				...
				if (_node.src && _node.src.indexOf) {
					for (var i = 0,
					_i = $R.skipStuffFromDomain__images.length; i < _i; i++) {
				...
				// }
				// 暂时忽略img的大小判断
				_result._is__image_medium = true; // 都是中型图片
				break;
			}
			...
			//  ===========
			for (var i = 0,
			_i = _node.childNodes.length; i < _i; i++) {
		...
		var _recursive = function(_node) {
			...
			case('iframe'):
				...
				if (_skip);
					...
					//  loop
					for (var i = 0,
					_i = $R.keepStuffFromDomain__video.length; i < _i; i++) {
			...
			if ($R.parsingOptions._elements_ignore_tag.indexOf('|' + _tag_name + '|') > -1);
				...
				if (_tag_name in $R.parsingOptions._elements_keep_attributes) {
					for (var i = 0,
					_i = $R.parsingOptions._elements_keep_attributes[_tag_name].length; i < _i; i++) {
			...
			//  =======
			switch (true) {
			case(($R.parsingOptions._elements_ignore_tag.indexOf('|' + _tag_name + '|') > -1)) : return;
			...
			if ($R.parsingOptions._elements_too_much_content.indexOf('|' + _tag_name + '|') > -1) {
				_explored = (_explored || $R.getContent__exploreNodeAndGetStuff(_node, true));
				switch (true) {
				case(_tag_name == 'h1' && (_explored._length__all_text > (65 * 2))) : case(_tag_name == 'h2' && (_explored._length__all_text > (65 * 2 * 3))) : case((_tag_name.match(/^h(3|4|5|6)$/) != null) && (_explored._length__all_text > (65 * 2 * 5))) : case((_tag_name.match(/^(b|i|em|strong)$/) != null) && (_explored._length__all_text > (65 * 5 * 5))) :
			...
			//  ==============
			switch (true) {
			case(($R.parsingOptions._elements_self_closing.indexOf('|' + _tag_name + '|') > -1)) : case(($R.parsingOptions._elements_ignore_tag.indexOf('|' + _tag_name + '|') > -1)) : case(_tag_name == 'td') : break;
			...
			default:
				...
				case (_contentsLength == 0) : case((_contentsLength < 5) && ($R.parsingOptions._elements_visible.indexOf('|' + _tag_name + '|') > -1)) :
	...
	$R.getContent__find__getIsolatedTitleInHTML = function(_html) {
		//  is it there?
		if ($R.getContent__find__hasIsolatedTitleInHTML(_html));
		else {
			return '';
		}

		//  regex
		var _getTitleRegex = new RegExp($R.articleTitleMarker__start + '(.*?)' + $R.articleTitleMarker__end, 'i'),
	...
	$R.getContent__find__isolateTitleInHTML = function(_html, _document_title) {
		...
		_i = _heading_pregs.length; i < _i; i++) {
			...
			//  return?
			switch (true) {
			case(!(_match)) : case(!(_heading_pregs[i].lastIndex > -1)) :
				//  will continue loop
				break;
			...
			default:
				...
				//  return?
				switch (true) {
				case(!(_heading_length > 5)) : case(!(_heading_length < (65 * 3))) : case(!(_to_heading_length < (65 * 3 * 2))) :
				...
				case ((_secondary_headings.indexOf('|' + _heading_type + '|') > -1)) :
					...
					case((_heading_length < 25) && (_matched_words.length < Math.ceil(_heading_length * 0.75))) : case((_heading_length < 50) && (_matched_words.length < Math.ceil(_heading_length * 0.65))) : case((_matched_words.length < Math.ceil(_heading_length * 0.55))) :
				...
				default:
					...
					+ $R.articleTitleMarker__start + _heading_text + $R.articleTitleMarker__end
	...
	$R.getHTMLByURL = function (url,cb) {
		$.get(url,function(data) {
			var a = $($R.document.documentElement).html($(data).find("body,head").parent().html());
			var res = $R.getContent__find();
			if (res) {
				// $R.articleTitle 纯文本标题
				// _my_FoundHTML带标题和内容在内的html代码
				cb($R._my_FoundHTML,$R.articleTitle,$R.document);
			};
		});
	}
